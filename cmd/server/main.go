...
func main() {
	...
	case "1\n":
		...
		item, err := c.CreateItem(ctx, &pb.User{FName: n, Id: i, city: c, phone: p , married: m,
			Category: int32(catInt)})
		if err != nil {
			log.Fatalf("Could not create a new item: %v", err)
		}
		fmt.Println("\nInserted", n, "with the ID", item.Id, " , " City" , "Phone" , "maried" , "height" , and category", catInt)
    ...
    fmt.Println("city:", read.city)
    fmt.Println("phone:", read.phone)
    fmt.Println("height:", read.height)
    fmt.Println("married:", read.married)
